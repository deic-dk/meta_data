    ...
    if( $('#newKey').val() ){
      $.ajax({                                                                                                                   
        ...
        success: function(result) {
          ...
          $('#newKey').val('');
          $('#keyTable').append('<tr data-tag="'+ tag + '" class="keyRow visible"><td>'+key+'</td><td><input type="text" value="" data-key="'+ key +'" data-keyid="'+array[0]['keyid']+'"></td></tr>');            
        },                                                                                                                       
...
function loadFileTags(fileID){
  var tokens = [];
  $.ajax(  {
    url: OC.filePath('meta_data', 'ajax', 'loadfiletags.php'),
    async: false,
    timeout: 2000,
...
function loadKeysAndFileKeys(taglist, fileID){
  ...
  $.ajax({
    url: OC.filePath('meta_data', 'ajax', 'loadfilekeys.php'),
    async: false,
    timeout: 2000,
...
function updateFileTags(eventData, fileID){
  $.ajax({
    url: OC.filePath('meta_data', 'ajax', 'updatefiletags.php'),
    async: false,
    timeout: 2000,

    data: {
      tag: eventData.attrs.label,
      fileid: fileID
    },
    type: "POST",
    success: function(result) {
...
function updateFileKeys(file){
  ...
  $('tr.keyRow').each( function() {
    ...
    if( jQuery.inArray( tag, tagArray) > -1) {
      ...
      $.ajax({                                                                                                                       
            url: OC.filePath('meta_data', 'ajax', 'updatefilekeys.php'),                                                                 
            async: false,                                                                                                                
            timeout: 2000,                                                                                                               
