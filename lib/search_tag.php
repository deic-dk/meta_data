...
class Tag extends \OC_Search_Provider {
  ...
  function search($query) {
	$tags = tags::searchTag($query."%",User::getUser(), "%");

	$results = array();
	foreach ($tags as $tagData) {
	  $result['type'] = 'tag';
	  $result['name'] = $tagData['descr'];
	  $result['link'] = "/index.php/apps/files?dir=%2F&view=tag-".$tagData['tagid'];
...
class Metadata extends \OC_Search_Provider {
  ...
  function search($query) {
	...
	foreach ($tags as $tagData) {
	  ...
	  $tagInfo = tags::searchTagbyID($tagData['tagid']);
	  $keyInfo = tags::searchKeybyID($tagData['keyid']);

	  $result['type'] = 'metadata';
	  $result['name'] = $fileInfo['name'];
	  $result['text']  = "<span class='label outline'><i class='icon-tag'></i>". $tagInfo['descr']."</span>".$keyInfo['descr']."=".$tagData['value'];
